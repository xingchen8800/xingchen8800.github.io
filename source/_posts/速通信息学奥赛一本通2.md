...
设走 $i$ 步，最后一步向左走的的走法有 $l_i$ 种，向上走有 $u_i$ 种，向右走有 $r_i$种，则共有 $f_i = l_i+r_i+u_i$ 种走法。
...
### 最优方案——[树状数组](https://xingchen8800.github.io/2024/08/17/d10/)+离散化
...
对于每个数 $i$，我们都要求他之前的比它小的数的个数，当我们将树状数组中的`sum(1,i-1)`作为查询有多少个比 $i$ 小的数时，只需要每次查询`sum(1,i-1)`并累加，将原数组作为桶维护树状数组即可。

```c++
#include <bits/stdc++.h>
using namespace std;

int a[100001], rank[100001], t[100001];	//rank是离散化后的结果，rank[i]表示i的离散化结果 
int n;
